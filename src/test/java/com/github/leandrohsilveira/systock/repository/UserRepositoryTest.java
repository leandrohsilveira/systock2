...
public class UserRepositoryTest {
	...
	public void postTest() throws Exception {
		String userJson = new JSONObject().put("username", "silveira").put("password", "silveira").toString();
		
		printSeparator();
		logger.info("Posting user: {}", userJson);
		printSeparator();
			
		MockHttpServletRequestBuilder post = post(new URI("/users"));
	...
	public void getTest() throws Exception {
		...
		MockHttpServletRequestBuilder get = get(new URI("/users/".concat(savedUser.getId())));
	...
	public void putTest() throws Exception {
		...
		String userJson = new JSONObject().put("username", "silveira2").put("password", "silveira2").toString();
		
		printSeparator();
		logger.info("Putting user id {}: {}", savedUser.getId(), userJson);
		printSeparator();
			
		MockHttpServletRequestBuilder put = put(new URI("/users/".concat(savedUser.getId())));
		...
		assertThat(updatedUser.getUsername()).isEqualTo("silveira2");
	...
	public void putTestBadRequest() throws Exception {
		...
		MockHttpServletRequestBuilder put = put(new URI("/users/".concat(id)));
		put.content(userJson);
		ResultActions result = mockMvc.perform(put);
		result.andDo(print());
	...
	public void patchTest() throws Exception {
		...
		MockHttpServletRequestBuilder patch = patch(new URI("/users/".concat(id)));
